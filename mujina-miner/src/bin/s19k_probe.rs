...
async fn main() -> anyhow::Result<()> {
    ...
    let chain: u8 = args
        .next()
        .expect("usage: s19k-probe <chain 1|2|3> [baud]")
        .parse()
    ...
    let mut data_reader = FramedRead::new(reader, bm13xx::FrameCodec);
    ...
    if chips.len() == 77 {
        ...
        println!("Does NOT match expected 77 chips/chain (or timing/init needs tuning).");
    }
